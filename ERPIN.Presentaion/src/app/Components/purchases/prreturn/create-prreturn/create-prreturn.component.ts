...
@Component({
  ...
  templateUrl: './create-prreturn.component.html',
  styleUrl: './create-prreturn.component.css',
})
export class CreatePrreturnComponent {
  ...
  constructor(
    private invoiceService: PRReturnService,
  ...
  ngOnInit(): void {
    forkJoin([
      ...
      this.route.params.subscribe((params) => {
        if (params['id']) {
          this.isEdit = true;
          this.titleService.setTitle(this.translate.instant('PRReturn.Edit'));
          this.getInvoice(+params['id']);
        } else {
          this.isEdit = false;
          this.titleService.setTitle(this.translate.instant('PRReturn.Create'));
          this.getInvoice();
        }
      });
    });
  ...
  initInvoiceForm(invoice: PRReturnResponse) {
  ...
  Submit() {
    ...
    const createModel: CreatePRReturn = {
      id: formValue.id,
      code: formValue.code,
      note: formValue.note,
      ...
      next: (response) => {
        Swal.fire({
          ...
          if (this.isEdit) {
            ...
            this.router.navigate(['/PurchaseReturn', 'Edit', response.data]);
            this.printInvoice();
          }
        });
      },
        ...
        this.invoiceService.deleteInvoice(id).subscribe({
          next: (response) => {
            Swal.fire({
              ...
              this.router.navigate(['/PurchaseReturn', 'List']);
            });
          },
          error: (error) => {
            Swal.fire({
